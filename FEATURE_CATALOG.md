...
- Every mutating feature emits a typed post-event and an audit record. Rows name only the
  events a consumer would care about.
...
| RT-CORE-IDEM | Idempotency store: claim/complete/release/prune | RT-CORE-STORAGE | — | `rt_idempotency` | 1 | — | — | — | `IdempotencyStore` | **SHIPPED** |
...
| RT-CORE-RESIDENT | Resident identity, name normalisation, join/leave, last-seen | RT-CORE-STORAGE | RiftCore | `rt_resident` | 1 | `config.yml` | `/resident` `/res` · `rifttowny.resident.*` | G B C T | `ResidentAdmitted/Released`, `ResidentView` | **ACTIVE** — aggregate, name policy and JDBC repository done and tested; **no commands or UI yet** |
| RT-CORE-TOWN | Town lifecycle: found, rename, transfer mayoralty, disband, merge | RT-CORE-RESIDENT | RiftEco, RiftLogger | `rt_town`, `rt_town_trust` | 1 | `modules/town.yml` | `/town` `/t` · `rifttowny.town.*` | G B C T | `TownRenamed/LeadershipTransferred/TownJoinedNation/TownLeftNation` | **ACTIVE** — aggregate and JDBC repository done and tested; lifecycle service, commands and UI outstanding |
| RT-CORE-NATION | Nation lifecycle: create, join, leave, capital, transfer kingship, disband | RT-CORE-TOWN | RiftEco | `rt_nation` | 3 | `modules/nation.yml` | `/nation` `/n` · `rifttowny.nation.*` | G B C T | `NationCreated/CapitalMoved/TownJoined/TownLeft/Disbanded` | PLANNED |
| RT-CORE-CLAIM | Chunk claims, homeblock, outposts, contiguity, preview and dry-run | RT-CORE-TOWN | CoreProtect | `rt_claim` | 1 | `modules/claims.yml` | `/town claim|unclaim`, `/plot` `/p` · `rifttowny.claim.*` | G B C T | `ClaimCreated/Removed`, `ClaimView` | PLANNED |
| RT-CORE-AREA | 3D areas, non-overlap, containment, districts, plots | RT-CORE-CLAIM | — | `rt_area` | 2 | `modules/areas.yml` | `/plot area *` · `rifttowny.area.*` | G B C T | `AreaCreated/Removed`, `AreaView` | PLANNED |
| RT-CORE-PROTECT | Central flag resolver and every protection listener | RT-CORE-CLAIM, RT-CORE-ROLE | CoreProtect, RiftLogger | `rt_flag_override` | 1 | `modules/protection.yml` | `/town set flag`, `/townyworld` · `rifttowny.flag.*` | G B C T | `FlagDecision`, `flagResolver()` on the API | PLANNED |
| RT-CORE-ROLE | Configurable roles, priority management, system roles, action/management split | RT-CORE-TOWN | LuckPerms | `rt_role`, `rt_role_permission`, `rt_role_member` | 1 | `modules/roles.yml` | `/town role *`, `/nation role *` · `rifttowny.role.*` | G B C T | `RoleChanged`, `RoleView` | **ACTIVE** — domain, storage and `TownRoleService` done and tested, including escalation guards; **nation roles, commands and UI outstanding** |
| RT-CORE-CMD | Command framework, tab completion, clickable chat components | RT-CORE-CONFIG | — | none | 1 | — | — | C T | — | PLANNED |
...
| ID | Feature | Requires | Optional | Storage | Era | Config | Commands / Permissions | UI | API / Events | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| RT-MOD-BANK | Civic banks, multi-currency, deposits, withdrawals, history, disband settlement | RT-CORE-TOWN | **RiftEco** (preferred), VaultUnlocked | `rt_organisation_currency`, `rt_bank_ledger` | 1 | `modules/bank.yml` | `/town bank *`, `/nation bank *` · `rifttowny.bank.*` | G B C T | `CivicDeposit/Withdrawal/Settled` | PLANNED |
...
| RT-MOD-REGEN | Wilderness snapshot, gradual restore, persistent queue, explosion regen | RT-CORE-CLAIM | CoreProtect, RiftShop, RiftSpawners | `rt_regen_queue`, `rt_regen_snapshot` | 2 | `modules/regeneration.yml` | `/townyadmin regen *` · `rifttowny.admin.regen` | G B C T | `RegenQueued/Completed` | PLANNED |
...
| RT-MOD-FLIGHT | Territory flight eligibility, handoff, no-fly zones, **Soft Landing** | RT-CORE-PROTECT | **RiftBoosters**, RiftPvP | `rt_flight_grant` | 2 | `modules/flight.yml` | `/tfly` `/townyfly` `/townyflight` · `rifttowny.flight.*` | G B C T | `FlightGranted/Revoked`, `SoftLandingStarted` | PLANNED |
| RT-MOD-CHAT | Town/nation/ally channels through RiftChat | RT-CORE-TOWN | **RiftChat**, VelocitySrv | `rt_chat_pref` | 1 | `modules/chat.yml` | `/tc` `/nc` `/ac` · `rifttowny.chat.*` | C T (G B for settings) | context supplier for RiftChat | PLANNED |
...
| RT-MOD-MAP | One abstraction over BlueMap, Dynmap, squaremap | RT-CORE-CLAIM | three map plugins | none | 1 | `modules/maps.yml` | `/towny map` · `rifttowny.map` | C T | `MapMarkerProvider` | PLANNED |
| RT-MOD-PAPI | `%townyadvanced_*%` + `%rifttowny_*%`, snapshot-cache only | RT-CORE-TOWN | **PlaceholderAPI** | none | 1 | `modules/placeholders.yml` | — | — | versioned manifest | PLANNED |
...
| RT-MOD-MIGRATE | Import adapters for Towny, Lands, HuskTowns; SQLite ⇄ MariaDB transfer | RT-CORE-STORAGE | — | `rt_migration_run` | 1 | `modules/migration.yml` | `/rifttowny migrate *` · `rifttowny.admin.migrate` | C T | — | PLANNED |
