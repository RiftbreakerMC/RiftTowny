...
| RT-MOD-TAX | Resident tax, town tax, nation tax, upkeep, bankruptcy | RT-MOD-BANK | RiftEco | `rt_tax_run` | 2 | `config.yml` (`taxes.*`) | — · `rifttowny.tax.*` | G B C T | `TaxRunCompleted`, `Bankruptcy` | **ACTIVE** — the engine runs: `TaxService` charges residents, towns and nations on a schedule, upkeep scales with land, a town that cannot pay is marked and eventually falls, and a run interrupted part-way resumes without charging anybody twice. Rates are server-wide in `config.yml`. **Outstanding**: no per-town lever — there is no `/town set taxes` and no `rt_tax_schedule`, which is why `Permission.MANAGE_TAXES` is grantable and checked by nothing |
