...
| `rt_role.chat_prefix` in chat | Settable and visible in `/town role list`, but never rendered in `/tc` or `/nc` | Same blocker as `Permission.CHAT_TOWN`: `ChannelRenderer` runs inside `AsyncChatEvent` and holds no role lookup, and `CivicCache` carries towns, membership and trust but not roles. Both unblock together, on a cached role book with invalidation across all nine role mutations |
