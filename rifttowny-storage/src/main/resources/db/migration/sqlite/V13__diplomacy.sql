...
CREATE TABLE rt_nation_relation (
    ...
    FOREIGN KEY (declarer_id) REFERENCES rt_nation (nation_id) ON DELETE CASCADE,
    FOREIGN KEY (target_id)   REFERENCES rt_nation (nation_id) ON DELETE CASCADE
);

-- Answering "who has declared anything about me" without scanning: the target side is the one with
-- no natural index, and it is what a nation's own relations screen reads.
CREATE INDEX idx_rt_nation_relation_target ON rt_nation_relation (target_id, relation);
