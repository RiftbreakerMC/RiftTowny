...
CREATE TABLE rt_flag_override (
    ...
    -- Resolution reads every opinion for one target at a time, and the primary key already leads
    -- with (scope, target). This second index serves the other direction: finding every override a
    -- town or world holds, for a listing and for the sweep when one is deleted.
    INDEX idx_rt_flag_override_target (target)
