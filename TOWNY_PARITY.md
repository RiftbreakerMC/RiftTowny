...
| Custom ranks with configurable permissions | `/town role new`, `delete`, `grant`, `revoke`, `priority`, `set`, `assign`, `unassign` | DONE — Towny's ranks are config-defined; ours are per-town and editable in game. Until 2026-08-26 this row overstated it: `grant`, `revoke` and `priority` existed as service methods with no command, so a town could create a rank and never set what it did |
