...
- [ ] **Spawn travel has no warmup, cooldown or cost** — a player can chain `/town spawn` to escape a fight, and nothing charges for it. Warmup and cooldown want `RT-MOD-SAFELOC`'s shape and a combat-tag signal from RiftPvP; the cost wants `RT-MOD-BANK`. Public and nation spawns are unbuilt too: only your own town's works
