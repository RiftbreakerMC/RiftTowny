...
- [ ] **`RT-MOD-CHAT`** — `/tc` and `/nc`. RiftTowny selects recipients, RiftChat renders: `RiftChatService.Channel` already has `TOWN` and `NATION`, so nothing upstream is in the way. `/ac` stays blocked until `RT-MOD-DIPLOMACY` gives it allies to send to
