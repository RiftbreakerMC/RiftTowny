...
- [x] **`RT-MOD-DIPLOMACY`** — `V13`, declarations stored one way so an alliance needs both nations and an enmity needs one, the ALLY rung in protection reading it, and `/nation relations ally|enemy|neutral|list`. **Outstanding**: the ally/enemy placeholders and the relational colour still return blank, and `/ac` still has no channel constant in RiftChat
