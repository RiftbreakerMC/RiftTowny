...
- [~] **Protection gaps to close** — the block-side holes are closed; two remain. **Done**: families are matched by vanilla tag rather than by name, so signs (all 48), shelves, copper chests, beehives, cave vines, flower pots and campfires are covered and a wood type added in a later release arrives already protected. A spawner now answers `SPAWNER_USE` rather than the switch permission — that flag had existed since the ladder was written and nothing in the Paper module had ever reached it. A dragon egg is caught on the left-click that teleports it, since it fires no break event. `EntityPlaceEvent` and `HangingPlaceEvent` are listened to, so hanging a painting or setting down a boat inside a claim is a `BUILD` again rather than a hole beside the wall. And the tool in hand is judged with the block: stripping a log, scraping or waxing copper, pathing grass, tilling dirt and carving a pumpkin all rewrite a town's blocks with no break or place event, and none of them could ever have been caught by a list of materials. **Still open**: no area overrides (`AREA` is a declared layer with no source, and needs `RT-CORE-AREA`), and no operator command for the world and admin layers — `FlagService.setAdministrative` is reached by one test and nothing else, `clearAdministrative` by nothing at all, so both layers resolve on every check and can hold no rows
...
- [x] **The read-only surface** — `/resident [player]`, `/town info [town]`, `/town list`, `/town online`, `/town map`, `/nation info [nation]`, `/nation list`. No new table and no new query on the hot paths: the listings are a read model (`CivicDirectory`, `TerritoryMap`) over the caches protection already reads, so looking at a town costs nothing and a curious player cannot stall a server. The two things a cache cannot answer — a treasury balance and a nation's *name* rather than its id — are read once for the whole screen and rendered in their callback, so the lines never arrive in a different order on a slower database. The map draws two channels: **colour says whose**, **shape says what**, and it stays readable through Geyser because neither hover nor click carries information the text does not
