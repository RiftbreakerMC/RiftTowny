...
- [x] **`/resident set notices`** — `V15` adds `rt_resident_preference`. **One mode, and the scope is the point**: a survey of every message path found the territory notice is the only unsolicited, repeating, per-player message the plugin sends, so it is the only thing there is to turn off. Map modes, plot borders and auto-claim have nothing behind them; spy is permanently excluded; persisting the chat channel turned out to be a written refusal in `ActiveChannels` and `FEATURE_CATALOG` rather than a gap. **Unset is the absence of a row**, so a player who never chose follows the server while one who chose `off` stays off when the operator changes it - the same argument `V12` makes for a town's map colour, taken one step further
