...
- [~] **`RT-MOD-BANK`** — `V10` adds `rt_organisation_balance` and `rt_bank_ledger`. `Money` is `BigDecimal` because a ledger of doubles loses money; the ledger is ordered by a sequence rather than a timestamp because two movements in the same millisecond are ordinary. `/town bank`, `/town deposit`, `/town withdraw`, and a treasury line on `/town info`. **The civic half is done and works with no economy plugin**; the player half is `BLOCKED` — RiftEco is not in this workspace, `PlayerWallet.absent()` is the only implementation, and both transfer commands refuse with `NO_ECONOMY`. Contract in [INTEGRATION_CONTRACTS.md](INTEGRATION_CONTRACTS.md) §2.7

### Next, in the order [TOWNY_PARITY.md](TOWNY_PARITY.md) argues for

- [ ] **Spend the bank** — claim costs, town founding cost, the ruin reclaim price, plot prices and spawn travel cost. All of these are now one `BankService.charge` call away, and all of them are already written up as outstanding elsewhere in this file
- [ ] **`RT-MOD-TAX`** — resident tax, town tax, nation tax, upkeep, bankruptcy. Needs a scheduled run and the bank, which now exists
