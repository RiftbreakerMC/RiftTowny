...
- [~] **Prices** — `CivicPrices` reads six from configuration, all zero by default, and claiming and unclaiming charge and refund the treasury **inside the same transaction as the claim**, so a town can never own a chunk it did not pay for. **Founding, plot, reclaim and spawn-travel prices are configured and not yet charged** — each needs the wallet on a path that currently has no bank service

### Next, in the order [TOWNY_PARITY.md](TOWNY_PARITY.md) argues for

- [ ] **Charge the remaining four prices** — founding, plots, reclaim and spawn travel. All are read from configuration and none is taken yet, which is worse than not offering them; either wire them or remove them from `config.yml`
