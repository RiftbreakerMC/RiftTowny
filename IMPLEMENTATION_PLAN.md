...
- [ ] **A plot notice cannot name an offline holder** — the border notice reads from memory and must not wait on storage, so somebody else's plot reads as "another resident" unless they are online. A resident-name cache would fix it and is wanted by `/plot info` too
